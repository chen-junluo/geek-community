...
EVALUATION_PROMPT = """你是一位中文技术问答社区的内容关系评估专家。请根据同一个问题下的两个答案，评估 later answer 相对于 earlier answer 的"technical-path deviation"（技术路径偏离）程度。

核心定义：
"技术路径偏离"指 later answer 是否偏离 earlier answer 的核心诊断、解释、方法或实现方案，转向另一条不同的技术路径。它衡量的是两个答案之间的关系，不是 later answer 本身的质量、正确性、长度、礼貌程度或可读性。

请先识别 earlier answer 的核心技术路径，再判断 later answer 与它的关系。核心技术路径包括：主要原因判断、主要技术方案、关键操作步骤、排查方向、代码实现思路、工具或框架选择、以及对问题机制的解释。

评分标准如下：
- 0 分：later answer 基本仍沿着 earlier answer 的核心技术路径展开，只是在同一路径上补充细节、条件、代码、例子、边界情况或执行步骤，没有明显转向。
- 1-3 分：later answer 只有轻微偏离。主体仍在 earlier answer 的原路径上，但加入了少量不同的补充方向、替代性细节或次要调整。
- 4-6 分：later answer 有中等程度的偏离。它部分保留 earlier answer 的核心方向，但同时引入了新的主要方法、不同的排查重点、不同的实现方案，或把回答重心转向另一条较重要的路径。
- 7-9 分：later answer 明显偏离 earlier answer 的核心技术路径。它主要提出另一套诊断、方案、工具/框架、实现方式或排查路线，只保留很少的原路径痕迹，或仅在问题主题层面相关。
- 10 分：later answer 几乎完全脱离 earlier answer 的原本技术路径，转向完全不同的核心解决路径，或基本没有回应 earlier answer 所涉及的解决方向。

判断时请遵守以下规则：
1. 必须结合 Question 理解两个答案是否在解决同一个具体问题。
2. 不要把答案质量当成评分对象。一个更正确或更详细的 later answer，如果仍然走的是同一路径，也不应给高分。
3. 不要把文本相似度当成评分对象。措辞不同但核心路径一致，应给低分；措辞相似但核心方案转向了别的路径，也可以给高分。
4. 不要因为 later answer 更长就自动给高分。只有当新增内容意味着 later answer 改变了 earlier answer 的核心诊断、方法、实现路径、工具选择或排查方向时，才提高分数。
5. 如果 later answer 同时包含延续原路径和转向新路径的内容，请根据其主要贡献给分：主要仍在原路径上则偏低，主要转向新路径则偏高，二者接近则给 4-6 分。
6. 如果 earlier answer 或 later answer 内容过短、含糊或缺乏实质解决方案，请根据可识别的路径信息谨慎评分，并在 justification 中说明原因。
7. 请给出一个 0-10 的整数分数。
8. justification 必须是简单的一句中文说明，说明 later answer 是如何保持或偏离 earlier answer 的核心技术路径。
9. relationship_label 必须返回，且只能是 same_path、different_path、mixed 三者之一，不允许留空，不允许返回其他词。
10. 评分和 relationship_label 必须一致：0-3 分优先对应 same_path，4-6 分优先对应 mixed，7-10 分优先对应 different_path。如二者不一致，请以这个映射修正 relationship_label。
...
def _load_cache(cache_key: str) -> Optional[Dict]:
    ...
    try:
        ...
        if data.get("error_reason"):
            return None
        score = data.get("deviation_score")
        try:
            score = int(score) if score is not None else None
        except Exception:
            score = None
        label = _normalize_relationship_label(score, data.get("relationship_label"))
        if "deviation_score" in data and "justification" in data and label is not None:
            data["deviation_score"] = score
...
def evaluate_single_pair(row: pd.Series) -> dict:
    ...
    for field, label in [("earlier_answer_text", "earlier answer"), ("later_answer_text", "later answer")]:
        if pd.isna(row[field]) or not str(row[field]).strip():
            return {"cache_key": cache_key, "deviation_score": None, "justification": None,
    ...
    try:
        score = max(0, min(10, int(parsed.get("deviation_score"))))
    except Exception:
        score = None

    label = _normalize_relationship_label(score, parsed.get("relationship_label"))
    if label is None:
        return {"cache_key": cache_key, "deviation_score": score, "justification": parsed.get("justification"),
                "relationship_label": None, "error_reason": "relationship_label 缺失", "raw_response": raw_response}

    result = {"cache_key": cache_key, "deviation_score": score, "justification": parsed.get("justification"),
...
def process_single_pair(row: pd.Series) -> dict:
    try:
        ...
        return {"cache_key": row["cache_key"], "deviation_score": None, "justification": None,
...
def build() -> pd.DataFrame:
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        with tqdm(total=len(pair_records), desc="LLM Deviation Evaluation") as pbar:
            for future in as_completed(future_to_key):
                try:
                    eval_results.append(future.result())
                except Exception as e:
                    eval_results.append({"cache_key": future_to_key[future], "deviation_score": None,
