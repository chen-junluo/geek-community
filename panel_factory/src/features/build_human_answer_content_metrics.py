...
#   - Core: textLength, textLengthCN, codeLength, imgNum, brNum, codeNum, inlinecodeNum, interlinecodeNum, hrefNum, aNum, boldNum, italicNum, ulNum, olNum, liNum, blockquoteNum, hrNum, tableNum
#   - Derived: —
#
# Logic:
...
#   - 输出不额外添加 `human_answer_` prefix，由 downstream builder 按需要解释语义
...
def build() -> pd.DataFrame:
    ...
    metrics_list = []
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing content"):
...
if __name__ == "__main__":
    ...
    print("\nText Length Metrics:")
    for col in ['textLength', 'textLengthCN', 'codeLength']:
    ...
    for col in sorted(formatting_cols):
        if col not in ['textLength', 'textLengthCN', 'codeLength']:
