# Artifact:  feature/answer_accepted_answer_similarity
# 输入:      data/features/human_answer_intermediate.csv
# Grain:     human-answer-level (questionURL × resp_id)
# Merge keys: questionURL, resp_id
# 输出:      data/features/answer_accepted_answer_similarity.csv
#
# 逻辑：用 accepted human answer 作为 anchor，计算每个 human answer 与 anchor 的
...
OUTPUT_CSV = ARTIFACT_PATHS["features"]["answer_accepted_answer_similarity"]
...
def build() -> pd.DataFrame:
    ...
    model = _load_model()
    tqdm.pandas(desc="Computing human vs accepted similarity")
    ...
    print(f"answer_accepted_answer_similarity 已保存: {OUTPUT_CSV}  shape={feature.shape}")
    return feature
