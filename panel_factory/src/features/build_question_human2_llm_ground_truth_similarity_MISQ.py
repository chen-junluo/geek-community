# Artifact:    feature/question_human2_llm_ground_truth_similarity_MISQ
# Grain:       question
...
#
# Output:      data/features/question_human2_llm_ground_truth_similarity_MISQ.csv
#   - Index: questionURL
#   - Core: human2SimWithGT__*, has_ground_truth__*, gt_error_reason__*
#   - Derived: has_human2, human2_resp_id, human2_dateID, gt_provider__*, gt_prompt_version__*, gt_model_name__*
#
# Logic:
#   - 在 MISQ question universe 内，用 `resp_id` 选择第二个 human answer（human2）
#   - 只读取 `Opus` 与 `DeepSeek` 的现有 ground-truth cache，不做任何 model call
#   - 将每个模型的 ground truth 与 human2 做 semantic similarity，输出 single wide feature table
...
OUTPUT_CSV = ARTIFACT_PATHS["features"]["question_human2_llm_ground_truth_similarity_misq"]
...
def build_human2_lookup(full_answer: pd.DataFrame) -> pd.DataFrame:
    ...
    human_rows = human_rows.dropna(subset=["resp_id_num"])
    human_rows = human_rows.sort_values(["questionURL", "resp_id_num"])
    human_rows["human_rank"] = human_rows.groupby("questionURL").cumcount() + 1
    human2 = human_rows[human_rows["human_rank"] == 2].copy()
    human2["has_human2"] = 1
    human2["human2_resp_id"] = human2["resp_id_num"].astype(int)
    human2["human2_dateID"] = human2["dateID"]
    human2["human2_selection_rule"] = "second_min_resp_id"
    human2["human2_text"] = human2["answer_text"]
    return human2[["questionURL", "has_human2", "human2_resp_id", "human2_dateID", "human2_selection_rule", "human2_text"]]
...
def _initialize_output_columns(feature: pd.DataFrame) -> pd.DataFrame:
    for model_slug in MODEL_EXECUTION_ORDER:
        feature[f"human2SimWithGT__{model_slug}"] = np.nan
...
def build() -> pd.DataFrame:
    ...
    feature = feature.merge(build_human2_lookup(full_answer), on="questionURL", how="left")
    feature["has_human2"] = feature["has_human2"].fillna(0).astype(int)
    ...
    for model_slug in MODEL_EXECUTION_ORDER:
        ...
        sim_col = f"human2SimWithGT__{model_slug}"
        ...
        valid = (
            feature["has_human2"].eq(1)
            & feature[has_gt_col].eq(1)
            & feature["human2_text"].astype(str).str.strip().ne("")
            & feature[gt_text_col].astype(str).str.strip().ne("")
        )
        if valid.any():
            tqdm.pandas(desc=f"Computing human2 vs GT similarity [{model_slug}]")
            feature.loc[valid, sim_col] = feature.loc[valid].progress_apply(
                lambda row: compute_similarity(row["human2_text"], row[gt_text_col], similarity_model),
                axis=1,
            )

        feature = feature.drop(columns=[gt_text_col])

    output_cols = ["questionURL", "has_human2", "human2_resp_id", "human2_dateID", "human2_selection_rule"]
    for model_slug in MODEL_EXECUTION_ORDER:
        output_cols.extend([
            f"human2SimWithGT__{model_slug}",
