...
WRITE_WORD_OUTPUTS <- tolower(Sys.getenv("WRITE_WORD_OUTPUTS", "true")) %in% c("true", "1", "yes")
...
load_and_process_data <- function(
  ...
  tag_df <- read.csv(tag_path, header = TRUE, sep = ",", 
                     encoding = "GB18030", stringsAsFactors = FALSE)
...
load_and_process_answer_data <- function(
  ...
  update_variables <- function(mydata_answer, tag_path) {
    ...
    df <- read.csv(tag_path,
                   header = TRUE, sep = ",", encoding = "GB18030", stringsAsFactors = FALSE)
...
if (WRITE_WORD_OUTPUTS) {
  wordreg(models,
            stars = c(0.1, 0.05, 0.01, 0.001),
            file = "Projects/MISQ/MISQ round 2/analysis/outputs/summary260609_main results.docx", inline.css = FALSE, doctype = FALSE, html.tag = TRUE, head.tag = TRUE, body.tag = TRUE,
            digits = 3, dcolumn = TRUE, threeparttable = TRUE, fontsize = "tiny",
...
mydata_AI_high <- mydata_AI_high %>% left_join(mydata_answer_firstHumanAns_length %>% dplyr::select(questionURL, textLengthCN_firsthuman), by = "questionURL")
# log textLengthCN_firsthuman
mydata_AI <- mydata_AI %>% mutate(log_textLengthCN_firsthuman = log(textLengthCN_firsthuman + 1))
mydata_AI_high <- mydata_AI_high %>% mutate(log_textLengthCN_firsthuman = log(textLengthCN_firsthuman + 1))
...
##############################################
######## 260609 results may be helpful
##############################################
models <- list()
models[["DV: ai_human1_similarity"]] <- felm (ai_human1_similarity ~ AI+AI:(log_textLengthCNAI_fillna + AISimWithOpus47_fillna )
      + log_textLengthCN_ask + IimgNum_ask + IaNum_ask + IblockquoteNum_ask + ItableNum_ask + lingComp_score + techJargon_score + difficulty_score
      + category1 + category2 + category3 + category4 + category5 + category6 + category7 + category8 + category9
      + log_age + log_askBefore + log_resBefore + log_accumRep_ask + accumGold_ask + accumSilver_ask + accumCopper_ask +  asker_acceptedBefore_allsite_1m + asker_nActivityBeforeAsk_allsite_1m + asker_nActivityBeforeResp_allsite_1m + asker_nActivityBeforeComment_allsite_1m + asker_acceptedBefore_allsite_2m + asker_nActivityBeforeAsk_allsite_2m + asker_nActivityBeforeResp_allsite_2m + asker_nActivityBeforeComment_allsite_2m + asker_acceptedBefore_allsite_3m + asker_nActivityBeforeAsk_allsite_3m + asker_nActivityBeforeResp_allsite_3m + asker_nActivityBeforeComment_allsite_3m
      | dayofyear | 0 | 0, data = mydata_AI
...
models <- list()
models[["DV: experience"]] <- felm (Iexperience_m2 ~ AI+AI:(log_textLengthCNAI_fillna + AISimWithOpus47_fillna )
      + log_textLengthCN_ask + IimgNum_ask + IaNum_ask + IblockquoteNum_ask + ItableNum_ask + lingComp_score + techJargon_score + difficulty_score
      + category1 + category2 + category3 + category4 + category5 + category6 + category7 + category8 + category9
      + log_age + log_askBefore + log_resBefore + log_accumRep_ask + accumGold_ask + accumSilver_ask + accumCopper_ask +  asker_acceptedBefore_allsite_1m + asker_nActivityBeforeAsk_allsite_1m + asker_nActivityBeforeResp_allsite_1m + asker_nActivityBeforeComment_allsite_1m + asker_acceptedBefore_allsite_2m + asker_nActivityBeforeAsk_allsite_2m + asker_nActivityBeforeResp_allsite_2m + asker_nActivityBeforeComment_allsite_2m + asker_acceptedBefore_allsite_3m + asker_nActivityBeforeAsk_allsite_3m + asker_nActivityBeforeResp_allsite_3m + asker_nActivityBeforeComment_allsite_3m
      | answer_timing | 0 | questionURL, data = mydata_answer_firstHumanAns
)
models[["DV: insight"]] <- felm (Iinsight_m2 ~ AI+AI:(log_textLengthCNAI_fillna + AISimWithOpus47_fillna )
      + log_textLengthCN_ask + IimgNum_ask + IaNum_ask + IblockquoteNum_ask + ItableNum_ask + lingComp_score + techJargon_score + difficulty_score
      + category1 + category2 + category3 + category4 + category5 + category6 + category7 + category8 + category9
      + log_age + log_askBefore + log_resBefore + log_accumRep_ask + accumGold_ask + accumSilver_ask + accumCopper_ask +  asker_acceptedBefore_allsite_1m + asker_nActivityBeforeAsk_allsite_1m + asker_nActivityBeforeResp_allsite_1m + asker_nActivityBeforeComment_allsite_1m + asker_acceptedBefore_allsite_2m + asker_nActivityBeforeAsk_allsite_2m + asker_nActivityBeforeResp_allsite_2m + asker_nActivityBeforeComment_allsite_2m + asker_acceptedBefore_allsite_3m + asker_nActivityBeforeAsk_allsite_3m + asker_nActivityBeforeResp_allsite_3m + asker_nActivityBeforeComment_allsite_3m
      | answer_timing | 0 | questionURL, data = mydata_answer_firstHumanAns
)
models[["DV: alternative"]] <- felm (Ialternative_m2 ~ AI+AI:(log_textLengthCNAI_fillna + AISimWithOpus47_fillna )
      + log_textLengthCN_ask + IimgNum_ask + IaNum_ask + IblockquoteNum_ask + ItableNum_ask + lingComp_score + techJargon_score + difficulty_score
      + category1 + category2 + category3 + category4 + category5 + category6 + category7 + category8 + category9
      + log_age + log_askBefore + log_resBefore + log_accumRep_ask + accumGold_ask + accumSilver_ask + accumCopper_ask +  asker_acceptedBefore_allsite_1m + asker_nActivityBeforeAsk_allsite_1m + asker_nActivityBeforeResp_allsite_1m + asker_nActivityBeforeComment_allsite_1m + asker_acceptedBefore_allsite_2m + asker_nActivityBeforeAsk_allsite_2m + asker_nActivityBeforeResp_allsite_2m + asker_nActivityBeforeComment_allsite_2m + asker_acceptedBefore_allsite_3m + asker_nActivityBeforeAsk_allsite_3m + asker_nActivityBeforeResp_allsite_3m + asker_nActivityBeforeComment_allsite_3m
      | answer_timing | 0 | questionURL, data = mydata_answer_firstHumanAns
